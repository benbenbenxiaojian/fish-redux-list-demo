...
import '../component/reducer.dart';
import '../state.dart';

class CountAdapter extends DynamicFlowAdapter<CountState> {
  CountAdapter()
      : super(
            pool: <String, Component<Object>>{'count': CountComponent()},
...
class _ListConnector implements Connector<CountState, List<ItemBean>> {
  ...
  void set(CountState state, List<ItemBean> substate) {
    if (substate?.isNotEmpty == true) {
      state.counts =
          List<String>.from(substate.map((bean) => bean.data).toList());
    } else {
      state.counts = <String>[];
    }
  }
